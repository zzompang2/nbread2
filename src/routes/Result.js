...
function Result(props) {
  ...
  for (let i=0; i<subsets.length; i++) {
    ...
    for (let j=0; j<subset.length; j++)
      money.push(moneyToPay[subset[j]]);
        ...
        <div className="title">엔빵 완료!</div>
				<div className="payments">
					...
					{members.map(member => (
						<div key={member.id} className="table__row">
							<div className="table__name whiteBox">{member.name === "" ? `사람${member.id+1}` : member.name}</div>
							<div className="table__pay whiteBox">{moneyToPay[member.id]}</div>
						</div>
					))}
				</div>
				...
				<div>
					...
					<div className="moneyFlow">
						<div className="table__row">
							{/* <span className="bodyText table__id2">번호</span> */}
						...
						{moneyFlows.map((flow, idx) => (
							<div key={idx} className="table__row">
								{/* <div className="table__id2 whiteBox">{flow.from+1}</div> */}
								<div className="table__name whiteBox">{members[flow.from].name === "" ? `사람${members[flow.from].id+1}` : members[flow.from].name}</div>
								<div className="table__pay whiteBox">{flow.money}원</div>
								{/* <div className="table__id2 whiteBox">{flow.to+1}</div> */}
								<div className="table__name whiteBox">{members[flow.to].name === "" ? `사람${members[flow.to].id+1}` : members[flow.to].name}</div>
							</div>
						))}
					</div>
				</div>
